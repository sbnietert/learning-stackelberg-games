...
class RepeatedSSG:
    ...
    def __init__(self, ssg: SSG, follower_discount_factor: float, discounting_type: Literal["geometric", "hyperbolic"]="geometric"):
        self.ssg = ssg
...
# setting 1
def gen_simplex_SSG_1(n_targets: int) -> SSG:
    dimension = n_targets
...
# setting 2
def gen_simplex_SSG_2(n_targets: int, v_low: float, v_high: float) -> SSG:
    dimension = n_targets
    simplex_A = np.concatenate([-np.eye(dimension), np.ones((1, dimension))])
    simplex_b = np.concatenate([np.zeros((dimension, 1)), np.ones((1, 1))])
    simplex = Polytope(simplex_A, simplex_b)
