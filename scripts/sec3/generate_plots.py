...
for SETTING in [1, 2]:
    ...
    clinch_data = pd.read_json(
        f"results/sec3/clinch_{SETTING}.jsonl", lines=True
    ).values.tolist()
    security_search_data = pd.read_json(
        f"results/sec3/security_search_{SETTING}.jsonl", lines=True
    ).values.tolist()
    ...
    plt.plot(x, y, "o", color="C0", label="ClinchSimplex")
    plt.plot(x2, y2, "o", color="C1", label="SecuritySearch")
    plt.plot(x, fit_line, "-", color="C0", label=f"$y = c_1 N^{{{coeffs[0]:.2f}}}$")
    plt.plot(x2, fit_line2, "-", color="C1", label=f"$y = c_2 N^{{{coeffs2[0]:.2f}}}$")
    ...
    plt.tight_layout()
    plt.savefig(f"results/sec3/plot_combined_setting_{SETTING}.pdf")  # Save plot
    plt.show()
